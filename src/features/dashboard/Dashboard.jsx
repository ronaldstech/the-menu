...
function Dashboard({ user, token, onLogout, createRestaurant, listRestaurants, getRestaurant, updateRestaurant, deleteRestaurant, listCategories, getCategory, createCategory, updateCategory, deleteCategory, listMenus, getMenu, createMenu, updateMenu, deleteMenu, listMenuItems, getMenuItem, createMenuItem, updateMenuItem, deleteMenuItem, listTables, getTable, createTable, updateTable, deleteTable, listStaff, getStaff, createStaff, updateStaff, deleteStaff }) {
  ...
  const views = { dashboard: <DashboardHome firstName={firstName} />, saved: <SavedView />, restaurant: <RestaurantManager token={token} listRestaurants={listRestaurants} getRestaurant={getRestaurant} createRestaurant={createRestaurant} updateRestaurant={updateRestaurant} deleteRestaurant={deleteRestaurant} />, category: <CategoryManager token={token} listRestaurants={listRestaurants} listCategories={listCategories} getCategory={getCategory} createCategory={createCategory} updateCategory={updateCategory} deleteCategory={deleteCategory} />, menu: <MenuManager token={token} listRestaurants={listRestaurants} listCategories={listCategories} listMenus={listMenus} createMenu={createMenu} listMenuItems={listMenuItems} getMenuItem={getMenuItem} createMenuItem={createMenuItem} updateMenuItem={updateMenuItem} deleteMenuItem={deleteMenuItem} />, tableQr: <TablesManager token={token} listRestaurants={listRestaurants} listTables={listTables} getTable={getTable} createTable={createTable} updateTable={updateTable} deleteTable={deleteTable} />, staff: <StaffManager token={token} listRestaurants={listRestaurants} listStaff={listStaff} getStaff={getStaff} createStaff={createStaff} updateStaff={updateStaff} deleteStaff={deleteStaff} />, profile: <ProfileView user={user} /> }
  ownerFeatures.forEach(([key, label, icon]) => {
    views[key] = key === 'restaurants' ? views.restaurant : key === 'category' ? views.category : key === 'staff' ? views.staff : key === 'menu' ? views.menu : key === 'table-qr' ? views.tableQr : <OwnerSection title={label} icon={icon} description={`Keep your ${label.toLowerCase()} organized and ready for service.`} />
  })
